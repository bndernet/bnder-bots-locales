...
export class TSLocalizations_pt_br {
    static Localizations: TSLocalizationKeys = {
        ...
        webhookLogging: {
            ...
            ticket_updated: {
                title(id: string): string {
                    return `Ticket ${id} atualizado`;
                }
            },
            ticket_deleted: {
                title(id: string): string {
                    return `Ticket ${id} excluído`;
                }
            },
            project_created: {
...
Precisa de ajuda? [Fale com o suporte](https://bnder.net/discord) — estamos aqui para ajudar.`;
        ...
        mails: {
            footer: {
                ...
                company_contact: "E-mail: contact@bnder.net - Telefone: +49 511 80764975",
            ...
            ticket_notifications: {
                ...
                reporter_delivery_failed_note(failedRecipient: string): string {
                    return `A entrega do e-mail ao solicitante ${failedRecipient} falhou. O solicitante pode não ter recebido a atualização mais recente do ticket.`;
                },
                ...
                reporter_delivery_failed_original_subject(originalSubject: string): string {
                    return `Assunto original: ${originalSubject}`;
                },
                subject_prefix_sla_breach(stepLabel: string): string {
                    return `Violação de SLA do ticket: ${stepLabel}`;
                },
                headline_sla_breach(ticketTitle: string): string {
                    return `Violação de SLA: ${ticketTitle}`;
                },
                body_sla_breach(slaName: string, stepLabel: string, currentAge: string, targetAge: string): string {
                    return `O SLA "${slaName}" foi violado para ${stepLabel}.\nTempo atual: ${currentAge}\nMeta: ${targetAge}.`;
                },
                sla_step_first_response: "primeira resposta",
                sla_step_next_response: "próxima resposta",
                sla_step_resolution: "resolução",
                ...
                footer_brand_context(displayName: string): string {
                    return `Enviado para ${displayName} via Bnder.`;
                },
                footer_brand_context_link_prefix(displayName: string): string {
                    return `Enviado para ${displayName} via `;
                },
