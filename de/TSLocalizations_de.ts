...
Benötigst du Hilfe? [Kontaktiere den Support](https://bnder.net/discord) — wir sind für dich da.`;
        ...
        mails: {
            footer: {
                ...
                company_contact: "E-Mail: contact@bnder.net - Telefon: +49 511 80764975",
            ...
            ticket_notifications: {
                ...
                reporter_delivery_failed_note(failedRecipient: string): string {
                    return `Die E-Mail-Zustellung an den Meldenden ${failedRecipient} ist fehlgeschlagen. Der Meldende hat das letzte Ticket-Update möglicherweise nicht erhalten.`;
                },
                ...
                reporter_delivery_failed_original_subject(originalSubject: string): string {
                    return `Ursprünglicher Betreff: ${originalSubject}`;
                },
                subject_prefix_sla_breach(stepLabel: string): string {
                    return `Ticket-SLA-Verletzung: ${stepLabel}`;
                },
                headline_sla_breach(ticketTitle: string): string {
                    return `SLA-Verletzung: ${ticketTitle}`;
                },
                body_sla_breach(slaName: string, stepLabel: string, currentAge: string, targetAge: string): string {
                    return `SLA "${slaName}" wurde für ${stepLabel} verletzt.\nAktuelles Alter: ${currentAge}\nZiel: ${targetAge}.`;
                },
                sla_step_first_response: "erste Antwort",
                sla_step_next_response: "nächste Antwort",
                sla_step_resolution: "Lösung",
                ...
                footer_brand_context_link_prefix(displayName: string): string {
                    return `Gesendet für ${displayName} über `;
                },
