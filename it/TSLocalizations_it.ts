...
export class TSLocalizations_it {
    static Localizations: TSLocalizationKeys = {
        task: {
            ...
            deadline_duration_due_now: "scade ora",
        ...
        event: {
            ...
            booking_reminder: {
                content(workspaceName: string, durationTillEventMinutes: number, location: string, start: string, end: string, timezone: string): string {
                    return `La tua prenotazione per ${workspaceName} inizia tra circa ${durationTillEventMinutes} minuti (${start} - ${end}, ${timezone}).${location}`;
                },
                title: "Promemoria prenotazione imminente",
                manage_bookings_button: "Gestisci prenotazione",
            }
        },
...
Hai bisogno di aiuto? [Contatta il supporto](https://bnder.net/discord) — siamo qui per te.`;
        ...
        mails: {
            footer: {
                ...
                company_contact: "E-mail: contact@bnder.net - Telefono: +49 511 80764975",
            ...
            ticket_notifications: {
                ...
                reporter_delivery_failed_note(failedRecipient: string): string {
                    return `La consegna dell’email al segnalatore ${failedRecipient} non è riuscita. Il segnalatore potrebbe non aver ricevuto l’ultimo aggiornamento del ticket.`;
                },
                reporter_delivery_failed_status(status: string): string {
                    return `Stato: ${status}`;
                },
                reporter_delivery_failed_diagnostic(diagnostic: string): string {
                    return `Diagnostica: ${diagnostic}`;
                },
                reporter_delivery_failed_original_subject(originalSubject: string): string {
                    return `Oggetto originale: ${originalSubject}`;
                },
                subject_prefix_sla_breach(stepLabel: string): string {
                    return `Violazione SLA ticket: ${stepLabel}`;
                },
                headline_sla_breach(ticketTitle: string): string {
                    return `Violazione SLA: ${ticketTitle}`;
                },
                body_sla_breach(slaName: string, stepLabel: string, currentAge: string, targetAge: string): string {
                    return `Lo SLA "${slaName}" è stato violato per ${stepLabel}.\nEtà attuale: ${currentAge}\nObiettivo: ${targetAge}.`;
                },
                sla_step_first_response: "prima risposta",
                sla_step_next_response: "risposta successiva",
                sla_step_resolution: "risoluzione",
                ...
                subject_mentioned(ticketTitle: string): string {
                    return `Menzione nel ticket: ${ticketTitle}`;
                },
                headline_mentioned(ticketTitle: string): string {
                    return `Sei stato menzionato in "${ticketTitle}"`;
                },
                body_mentioned(messagePreview: string): string {
                    return `Anteprima messaggio: ${messagePreview}`;
                },
                discord_dm_mentioned(ticketTitle: string, messagePreview: string, ticketUrl: string, ticketId: string): string {
                    return `Sei stato menzionato nel ticket "${ticketTitle}".\nAnteprima: ${messagePreview}\n\n[Apri ticket](<${ticketUrl}>)\nID: \`${ticketId}\``;
                },
            },
